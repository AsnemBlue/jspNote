...
<%
	...
	try{
		...
		//new DefaultFileRenamePolicy() 같은 이름이 있을 경우 rename1.jpg0>11.jpg
		Enumeration<String> paramNames = mRequest.getFileNames();	//파라미터이름들(%파일 이름 아님!)
		while(paramNames.hasMoreElements()){
			...
			System.out.print("파라이터이름 : "+param+", 서버에 저장된 파일 이름 : "+filename+" 사용자가 첨부한 오리지널 이름 : "+originalFilename);
		}
	}catch(IOException e){
		System.out.print(e.getMessage());
	}
	// 서버에 업로드 하자마자 소스폴더 file copy
	InputStream is = null;
	...
	try{
		...
		if(serverFile.exists()){ 
			...
			while(true){
				int nReadCnt = is.read(bs);	//읽어온 바이트 수
				if(nReadCnt==-1){
					break;
				}
