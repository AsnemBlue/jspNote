...
<% 
	...
	FileboardDto dto = new FileboardDto(fnum, cid, fsubject, fcontent, ffilename, fpw, 
			0, fref, fre_step, fre_level, fip, null, null, null);
	
	FileboardDao fDao = FileboardDao.getInstance();
	int result = FileboardDao.FAIL;
	if(fnum==0){ // 원글쓰기
		result = fDao.insertFileboardTotCnt(dto);
	}else{ // 답변글 쓰기
		result = fDao.reply(dto);
	}
	if(result==FileboardDao.SUCCESS){
		out.print("<h2>글등록성공</h2>");
	}
%>
<script>
	setTimeout(function(){
		location.href = 'fboardList.jsp?pageNum=1';
	},2000);
</script>
</body>
</html>