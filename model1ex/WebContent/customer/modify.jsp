...
<script>
	function chkJoinInfo(){
		if(frmJoin.cemail.value.length!=0 && ( 
				frmJoin.cemail.value.indexOf('@')<=0 ||
				frmJoin.cemail.value.lastIndexOf('@')==frmJoin.cemail.value.length-1 ||
				frmJoin.cemail.value.lastIndexOf('@')>frmJoin.cemail.value.lastIndexOf('.') 
		)){
			alert('메일을 확인하세요');
			frmJoin.cemail.focus();
			return false;
		}
	}
</script>
...
<body>
<%! String cid, cpw, cname, ctel, cemail, caddress, cgender; Date cbirth; Timestamp crdate; %>
<%
